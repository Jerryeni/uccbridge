...
class BridgeRelayer {
  ...
  async start() {
    ...
    logger.info('Using WebSocket + Polling hybrid mode for instant detection');
    
    // Start WebSocket listeners for real-time detection
    this.startWebSocketListeners();
    
    // Also run polling as backup (in case WebSocket fails)
    ...
    while (this.isRunning) {
      try {
        ...
        if (currentBlock > this.bscLastBlock) {
          ...
          // No delay when catching up, 1 second delay when caught up
          if (blocksBehind <= 1) {
            await this.sleep(1000);
          }
        } else {
          // Caught up, wait 1 second
          await this.sleep(1000);
        }
      } catch (error) {
        consecutiveErrors++;
        ...
        if (error.message.includes('rate limit') || error.message.includes('429')) {
          logger.warn('BSC RPC rate limit, switching endpoint...', { 
            consecutiveErrors,
          ...
          // Wait based on consecutive errors (exponential backoff)
  ...
  async handleUcBurn(event) {
    ...
    try {
      ...
      // STEP 3: Unlock on BSC with owner wallet (relayer key)
